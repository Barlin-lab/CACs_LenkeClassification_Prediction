import cv2
import torch
from data.draw_gaussian import *
...
def processing_test(image, input_h, input_w):
    ...
    out_image = image.astype(np.float32) / 1.
...
def draw_spinal(pts, out_image):
    colors = [(0, 0, 1023), (0, 1023, 1023), (1023, 0, 1023), (0, 1023, 0)]
    for i in range(4):
...
def rearrange_pts(pts):
    ...
    new_bboxes = []
    for sort_i in sort_tb:#所有脊椎从上到下排好
    ...
    return new_bboxes #最后每个脊椎的4个点排好，每段脊椎从上到下排好
...
def generate_ground_truth(image,
    ...
    for k in range(17):
        pts = pts_2[4*k:4*k+4,:]
        bbox_h = np.mean([np.sqrt(np.sum((pts[0,:]-pts[2,:])**2)), #左边长度
                          np.sqrt(np.sum((pts[1,:]-pts[3,:])**2))]) #右边长度
        ...
        radius = gaussian_radius((math.ceil(bbox_h), math.ceil(bbox_w))) #ceil向上取整 放回一个半径
        radius = max(0, int(radius))
        draw_umich_gaussian(hm[0,:,:], ct_int, radius=radius) #在heatmap中画出高斯分布
    ...
    ret = {'input': image,
           'hm': hm, #热图 [bs, 1 , 256, 128]
           'ind': ind, #中心点的位置 [bs, 17]
           'reg': reg, #offset []
           'wh': wh, #顶点 [bs, 17, 8]
           'reg_mask': reg_mask, #[bs,17]
           }
...
def processing_train(image, pts, image_h, image_w, down_ratio, aug_label, img_id):
    ...
    data_aug = {'train': transform.Compose([transform.ConvertImgFloat(),
                                            transform.PhotometricDistort(), #对3个channel进行变换
    ...
    # print(out_image)
    out_image = np.transpose(out_image / 1. - 0.5, (2,0,1))
    pts = rearrange_pts(pts)
    pts2 = transform.rescale_pts(pts, down_ratio=down_ratio) #根据输出结果放缩
