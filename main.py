...
def csvl_report():
    ...
    for cnt, data_dict in enumerate(data_loader):
        ...
        #     exit()
        # cv2.imwrite('./results/'+img_id, ori_image_regress)
        
        # if not os.path.exists('./results'):
        #     os.mkdir('./results')
        # path = os.path.join('./results/', img_id+'.jpg')
        # print(path)
        # cv2.imwrite(path, ori_image_points)
        pts3 = []
        for p in pts0:
            point1 = [p[2], p[3]]
            point2 = [p[4], p[5]]
        ...
        # sl = int((pts3[-3][0]/images.shape[3]) * 1000)
        # sr = int((pts3[-4][0]/images.shape[3]) * 1000)       
        # sry = pts3[-3][1]
        csvl = int((slx + srx) / 2)
        # csvl = int((csvl/images.shape[3]) * 1000)
        # print(csvl)
        ...
        # oimage = oimage.squeeze()
        # oimage = oimage.data.cpu().numpy()
        # oimage = oimage.transpose((1,2,0))
        # print(oimage.shape)
        # png_file = img_id + '.png'
        # output = Image.fromarray(oimage.astype(np.uint8))
        ...
        # print(oimage.shape)
        # oimage = Image.fromarray(oimage.astype(np.uint8))
        ...
        # oimage = oimage.resize((1000, 1000),Image.ANTIALIAS)
        # oimage = np.array(oimage).transpose(2,0,1)
        ...
        # print(simage)
        # simage = simage.data.cpu().numpy()[0]
        # # oimage = oimage.transpose(1,2,0)
        # simage = np.array(simage, np.uint8)
        # simage = Image.fromarray(simage.astype(np.uint8))
        # simage = simage.resize((1000, 1000),Image.ANTIALIAS)
        # simage = np.array(simage, np.uint8)
        ...
        # # print(type(oimage), oimage.shape, oimage.size, oimage.dtype)
        # # print(result.shape, oimage.shape)
...
def post_report():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    test_dataset = pre2post(data_root='./data/DATAEXPORT_TEST', train=False)
    test_dl = torch.utils.data.DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=2)
    # logging.info("- done.")
    ...
    for data_dict in test_dl:

        center1, hm1 = data_dict['points']
        center1, hm1 = center1.to(device), hm1.to(device)

        center1, hm1 = Variable(center1), Variable(hm1)
        ...
        # print(data_batch[0].data)
        # points = output_batch[0].cpu().data
        # pts3 = []
        # for i in range(70):
        #     point = [points[i*2].item(), points[i*2+1].item()] 
        result_dir = os.path.join('./results', patient_id)
        # for i in range(data_batch.shape[0]):
        pre = center1[0].cpu().data.view(17,2).numpy()
        # gt = labels_batch[0].cpu().data.numpy()
        post = pre_center[0].cpu().view(17,2).data.numpy()

        #反归一化
        # pre[1:140:2] = pre[1:140:2] * 2
        # gt[1:140:2] = gt[1:140:2] * 2
        # post[1:140:2] = post[1:140:2] * 2
        pre[:,1] = pre[:,1]*2
        post[:,1] = post[:,1]*2
        ...
        plt.subplot(131)
        # plt.subplot(131)
        z1 = np.polyfit(pre[:,1], pre[:,0], 4)
        p1 = np.poly1d(z1)  
        yvals=p1(pre[:,1])
        plt.plot(yvals, pre[:,1], 'r')
        plt.xlim(0, 1)
        plt.yticks([])
        plt.scatter(pre[:,0],pre[:,1],marker='x')

        # plt.subplot(132)
        # plt.scatter(gt[0:140:2], gt[1:140:2])

        # plt.subplot(133)
        # plt.scatter(post[0:140:2], post[1:140:2])
        plt.subplot(133)
        # print(post.shape)
        z1 = np.polyfit(post[:,1], post[:,0], 4)
        p1 = np.poly1d(z1)  
        yvals=p1(post[:,1])
        plt.plot(yvals, post[:,1], 'r')
        plt.xlim(0, 1)
        plt.yticks([])
        plt.scatter(post[:,0],post[:,1],marker='x')
